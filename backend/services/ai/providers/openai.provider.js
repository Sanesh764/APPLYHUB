...
class OpenAIProvider extends BaseProvider {
  ...
  async parseResume(rawText) {
    ...
    try {
      ...
      logger.error("OpenAI parseResume failed, returning mock fallback", err);
  ...
  async analyzeATS(parsedData, targetRole) {
    ...
    try {
      ...
      logger.error("OpenAI analyzeATS failed, returning mock fallback", err);
  ...
  async matchJob(parsedData, jobDescription) {
    ...
    try {
      ...
      logger.error("OpenAI matchJob failed, returning mock fallback", err);
  ...
  async generateCoverLetter(parsedData, companyName, jobTitle, jobDescription) {
    ...
    try {
      ...
      logger.error("OpenAI generateCoverLetter failed, returning mock fallback", err);
  ...
  async generateInterviewQuestions(parsedData, jobTitle, jobDescription) {
    ...
    try {
      ...
      logger.error("OpenAI generateInterviewQuestions failed, returning mock fallback", err);
